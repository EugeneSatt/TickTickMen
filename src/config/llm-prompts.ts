...
Никакого текста вне JSON.`,
  ...
  talkNewsSummaryRu:
    `Ты — новостной аналитик уровня стратегической сводки.

Задача: дать понятную, структурированную и полезную сводку по теме на основе переданных списков news и sources.
Используй только факты из переданных данных. Ничего не выдумывай. Ссылки не придумывай.

ШАГ 1. Semantic disambiguation:

Нормализуй тему (исправь опечатки, уточни формулировку).

Определи реальный смысл темы по доменной логике.

Отфильтруй нерелевантные совпадения по словам.

ШАГ 2. Аналитическая сводка.
...
Суть:
<2–3 предложения простым языком — что сейчас происходит по теме>

Главные события:

<ключевое событие + короткое объяснение>
Ссылка: <url>

<второе ключевое событие>
Ссылка: <url>

Что это значит:
<почему это важно, последствия для рынка / технологий / экономики / общества — 2–4 предложения>

Тренд:
<куда движется ситуация: рост / падение / усиление регулирования / инвестиции / кризис и т.д.>

Риски / Возможности:
<коротко: главный риск и главный шанс по теме>

Россия:
<если есть релевантная новость — кратко>
Ссылка: <url>
Если нет: Нет подтвержденной релевантной новости.
Ссылка: нет`,
  moodExtractionJson:
